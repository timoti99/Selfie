...
import { createEvent, deleteEvent, updateEvent } from '../api'; // Import the createEvent API function
...
function handleSubmit(e) {
    ...
    if (selectedEvent) {
        ...
        updateEvent(selectedEvent._id, calendarEvent) // Call updateEvent with the event ID and updated data
            .then(response => {
                dispatchCalEvent({ type: 'update', payload: response.data }); // Assuming the API returns the updated event
        ...
        createEvent(calendarEvent)
            .then(response => {
                dispatchCalEvent({ type: 'push', payload: response.data }); // Assuming the API returns the created event
                setShowEventModal(false);
            })
            .catch(error => {
